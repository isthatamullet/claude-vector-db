...
def run_enhanced_sync(progress_interval: int = 10):
    """Run enhanced conversation sync with full context awareness"""
    print("🧠 Enhanced sync mode - full context awareness enabled")
    print("  Features: All 7 Enhanced Components - Topic detection, quality scoring, adjacency analysis, feedback learning, troubleshooting context, real-time learning, validation patterns")
    
    # Initialize components (suppress repeated processor logging)
    ...
    for i, file_path in enumerate(jsonl_files, 1):
        ...
        try:
            # Extract entries with unified processor
            print("  🔍 Extracting with unified processor...")
            ...
            for entry_idx, entry_data in enumerate(all_entries):
                try:
                    # Use unified processor for consistent enhancement
            ...
            if enhanced_entries:
                ...
                file_realtime_learning = sum(1 for entry in enhanced_entries if hasattr(entry, 'realtime_learning_boost'))
                ...
                print(f"  📊 UNIFIED - Topics: {file_topics}, Solutions: {file_solutions}, Feedback: {file_feedback}, Adjacency: {file_adjacency}")
                print(f"  🧠 UNIFIED - Troubleshooting: {file_troubleshooting}, Learning: {file_realtime_learning}, Validation: {file_validation_learning}")
                
                # Add enhanced entries to vector database
                print(f"  💾 Adding {len(enhanced_entries)} UNIFIED enhanced entries to database...")
                ...
                print(f"  ✅ UNIFIED Database - Added: {added}, Skipped: {skipped}, Errors: {errors}")
            else:
                print("  ⚠ No valid entries found in this file")
    ...
    try:
        ...
        # Fallback if processor not defined
        processor_stats = {
    ...
    print("\n🎯 UNIFIED PROCESSOR Enhanced sync complete!")
    print(f"Files processed: {total_files}")
    print(f"Total entries added: {total_entries_added}")
    print(f"Total entries skipped: {total_entries_skipped}")
    print(f"Total errors: {total_entries_errors}")
    ...
    print("\n✨ All entries processed with UNIFIED PROCESSOR - identical logic as real-time hooks!")
...
def run_unified_sync(max_sessions: int = 50, enable_backfill: bool = True):
    ...
    print("🚀 Unified Enhancement sync mode - complete system optimization")
    ...
    try:
        ...
        if not recent_sessions:
            print("⚠️ No recent sessions found. Running enhanced sync instead...")
        ...
        print("\n✨ Unified enhancement processing complete with conversation chain optimization!")
        print(f"Completed at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        
    except Exception as e:
        print(f"❌ Error in unified sync: {e}")
        print("🔄 Falling back to enhanced sync...")
        run_enhanced_sync(progress_interval=5)


def run_migration_sync():
    """Run database migration from standard to enhanced format"""
    print("🔄 Migration sync mode - converting existing database to enhanced format")
    
    # This would implement migration logic
    # For now, we'll use enhanced sync as the migration path
    print("  Migration uses enhanced sync to rebuild database with full enhancements")
    run_enhanced_sync(progress_interval=5)


def main():
    parser = argparse.ArgumentParser(
        description="Enhanced Conversation Sync Script for Claude Code Vector Database",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python run_full_sync.py --unified         # Unified enhancement with conversation chain back-fill
  python run_full_sync.py --enhanced        # Full enhanced processing
  python run_full_sync.py --standard        # Standard processing only
  python run_full_sync.py --migrate         # Migration mode
  python run_full_sync.py                   # Default unified mode
        """
    )
    ...
    mode_group.add_argument(
        '--unified', 
        action='store_true', 
        help='Run with unified enhancement system (conversation chain back-fill, field optimization, health monitoring)'
    )
    mode_group.add_argument(
        '--enhanced', 
        action='store_true', 
        help='Run with full enhanced processing (topic detection, quality scoring, adjacency analysis, feedback learning)'
    )
    mode_group.add_argument(
        '--standard', 
        action='store_true', 
    ...
    mode_group.add_argument(
        '--migrate', 
        action='store_true', 
        help='Run database migration from standard to enhanced format'
    )
    ...
    print("🚀 Starting comprehensive conversation sync...")
