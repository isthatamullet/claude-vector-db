...
async def get_vector_db_health() -> Dict[str, Any]:
    """
    Comprehensive vector database health report excluding file watcher system
    
    Returns:
        Dict containing health metrics for hooks-based indexing system, 
        vector DB performance, storage status, and search capabilities
    """
    try:
        ...
        # Component 1: Vector Database Connectivity
        try:
            # Test basic connectivity with a lightweight query
            test_results = db.search_conversations(
                query="test connectivity",
                n_results=1,
                include_metadata=True
            )
            health_report["components"]["vector_db_connectivity"] = {
                "status": "healthy",
                "response_time_ms": (datetime.now() - start_time).total_seconds() * 1000,
                "can_query": True,
                "details": f"Successfully queried {len(test_results)} results"
            }
        except Exception as e:
            health_report["components"]["vector_db_connectivity"] = {
                "status": "unhealthy", 
        ...
        # Component 2: Storage Metrics
        try:
            # Get comprehensive storage statistics
            all_conversations = db.search_conversations(
                query="*",
                n_results=1000,  # Large limit to get total count
                include_metadata=True
            )
            ...
            for conv in all_conversations:
                ...
                if conv.get("has_code", False):
                    code_conversations += 1
            
            health_report["components"]["storage_metrics"] = {
                "status": "healthy",
                "total_conversations": len(all_conversations),
                "projects_tracked": len(projects),
                "code_conversations": code_conversations,
                "largest_project": max(projects, key=projects.get) if projects else None,
                "projects_breakdown": dict(sorted(projects.items(), key=lambda x: x[1], reverse=True)[:5])
            }
        except Exception as e:
            health_report["components"]["storage_metrics"] = {
                "status": "unhealthy",
        ...
        # Component 3: Search Performance Benchmark
        try:
            ...
            health_report["components"]["search_performance"] = {
                "status": "healthy" if search_duration < 1000 else "degraded",
                "search_response_time_ms": search_duration,
        ...
        # Component 4: Hooks-Based Indexing Status  
        try:
            # Check latest indexing from hooks logs (much simpler and more accurate)
            log_file = "/home/user/.claude/hooks/logs/response-indexer.log"
            
            if Path(log_file).exists():
                # Read last line of log file to get latest indexing timestamp
                with open(log_file, 'r') as f:
                    lines = f.readlines()
                
                latest_timestamp = None
                if lines:
                    # Parse last line for timestamp (format: 2025-07-27 08:44:02: Successfully indexed...)
                    ...
                    if ': ' in last_line:
                        ...
                        try:
                            ...
                            latest_timestamp = dt.strftime("%Y-%m-%dT%H:%M:%S.000Z")
                        except ValueError:
                            pass
                
                if latest_timestamp:
                    time_ago = format_time_ago(latest_timestamp)
                    
                    # Determine status based on recency
                    dt = datetime.fromisoformat(latest_timestamp.replace('Z', ''))
                    now = datetime.now()
                    ...
                    health_report["components"]["hooks_indexing"] = {
                        "status": status,
                        "last_indexed": latest_timestamp,
                        "time_since_last": time_ago,
                        "indexing_method": "hooks-based",
                        "details": f"Last indexing {time_ago} (from hooks logs)"
                    }
                else:
                    health_report["components"]["hooks_indexing"] = {
                        "status": "unknown",
                        "last_indexed": "none",
                        "time_since_last": "unable to parse log",
                        "details": "Could not parse hooks log timestamp"
                    }
            else:
                health_report["components"]["hooks_indexing"] = {
                    "status": "no_logs",
                    "last_indexed": "none", 
                    "time_since_last": "no log file found",
                    "details": "Hooks log file not found"
                }
        ...
        # Component 5: Enhanced Index Integrity Check
        try:
            # Use recent conversation method for more reliable sampling
            recent_conversations = await get_most_recent_conversation(limit=10)
            
            if recent_conversations.get('success') and recent_conversations.get('conversations'):
                sample_conversations = recent_conversations['conversations']
            else:
                # Fallback to direct collection access
                raw_sample = db.collection.get(limit=10, include=['documents', 'metadatas'])
                sample_conversations = []
                docs = raw_sample.get('documents', [])
                metas = raw_sample.get('metadatas', [])
                ids = raw_sample.get('ids', [])
            ...
            for conv in sample_conversations:
                if not conv.get("id"):
                    integrity_issues.append("Missing conversation ID")
                elif not conv.get("content") or len(str(conv.get("content", "")).strip()) == 0:
                    integrity_issues.append("Empty conversation content")
                elif not conv.get("timestamp"):
                    integrity_issues.append("Missing timestamp")
                else:
                    valid_conversations += 1
            
            health_report["components"]["index_integrity"] = {
                ...
                "integrity_issues": integrity_issues[:5],  # Limit to first 5 issues
                "integrity_score": valid_conversations / max(len(sample_conversations), 1)
            }
        except Exception as e:
            health_report["components"]["index_integrity"] = {
                "status": "unknown",
        ...
        # Component 6: Current Working Directory
        try:
            current_dir = Path.cwd()
            detected_project = detect_project_from_directory(current_dir)
            available_projects = list(get_project_mapping().keys())
            
            # Always report as healthy - current directory is always valid
            health_report["components"]["current_directory"] = {
                "status": "healthy",
                "working_directory": str(current_dir),
                "detected_project": detected_project if detected_project else "global/user",
                "available_projects_count": len(available_projects),
                "context": "Global tools and MCP setup" if not detected_project else f"Project: {detected_project}"
            }
        except Exception as e:
            health_report["components"]["current_directory"] = {
                "status": "unknown",
        ...
        # Count both "healthy" and "recent" as good status
        healthy_statuses = ["healthy", "recent"]
        healthy_count = sum(1 for status in component_statuses if status in healthy_statuses)
        total_components = len(component_statuses)
        
        if healthy_count == total_components:
            health_report["overall_status"] = "healthy"
        elif healthy_count >= total_components * 0.7:  # 70% threshold
        ...
        health_report["summary"] = {
            ...
            "health_check_duration_ms": total_duration,
            "recommendations": generate_health_recommendations(health_report["components"])
        }
        
        logger.info(f"Vector DB health check completed: {health_report['overall_status']}")
        ...
        return {
            ...
            "error": str(e)[:500],  # Limit error message
            "health_version": "2025-07-27"
        }

def format_time_ago(timestamp_str: str) -> str:
    """Format timestamp as time ago (e.g., '2 minutes ago', '1 day ago')"""
    try:
...
def generate_health_recommendations(components: Dict[str, Any]) -> List[str]:
    """Generate actionable health recommendations based on component status"""
    recommendations = []
    
    # Check vector DB connectivity
    if components.get("vector_db_connectivity", {}).get("status") != "healthy":
        recommendations.append("Vector database connection issues detected - check database service")
    
    # Check search performance
    search_perf = components.get("search_performance", {})
    if search_perf.get("status") == "degraded":
        response_time = search_perf.get("search_response_time_ms", 0)
        recommendations.append(f"Search performance degraded ({response_time:.0f}ms) - consider index optimization")
    
    # Check hooks indexing
    hooks_status = components.get("hooks_indexing", {})
    if hooks_status.get("status") == "no_data":
        recommendations.append("No conversations found in database - verify hooks system is active")
    elif hooks_status.get("status") == "stale":
        time_ago = hooks_status.get("time_since_last", "unknown")
        recommendations.append(f"Last conversation indexed {time_ago} - indexing may be inactive")
    elif hooks_status.get("status") == "recent":
        time_ago = hooks_status.get("time_since_last", "unknown")
        recommendations.append(f"Last indexing {time_ago} - system working normally")
    elif hooks_status.get("status") == "error":
        recommendations.append("Error checking indexing status - investigate hooks system")
    
    # Check index integrity
    integrity = components.get("index_integrity", {})
    if integrity.get("status") == "degraded":
        score = integrity.get("integrity_score", 0)
        recommendations.append(f"Index integrity issues found (score: {score:.2f}) - consider reindexing")
    
    # Check storage metrics
    storage = components.get("storage_metrics", {})
    if storage.get("status") == "healthy":
        total_convs = storage.get("total_conversations", 0)
        if total_convs > 10000:
            recommendations.append("Large conversation database - consider implementing archival strategy")
