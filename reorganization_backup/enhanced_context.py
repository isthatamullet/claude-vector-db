...
def is_solution_attempt(content: str) -> bool:
    """
    Determine if a message is a solution attempt from Claude.
    ...
    """
    solution_indicators = [
        "here's how", "try this", "you can", "let me", "i'll help",
        "solution", "fix", "implement", "update", "change", "modify",
        "use this", "run this", "add this", "replace", "install"
    ]
    ...
    has_code = any(marker in content for marker in ['```', 'function', 'const ', 'let ', 'npm ', 'git '])
    
    # Check for step-by-step instructions
    has_steps = bool(re.search(r'\d+\.\s', content)) or ('step' in content_lower)
    
    # Longer messages with technical content are more likely to be solutions
    is_substantial = len(content) > 200
    
    return (has_solution_language and (has_code or has_steps or is_substantial))
